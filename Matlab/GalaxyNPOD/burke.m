...
%%   
%%  P: minimize phi(Psi*lam) 
%%
%%     subject to lam in Delta
%%
%%where 
...
%%
%%        mu    and     norm(w.*Psi*lam - erow,infinity)
%%
...
%%%%%%%%%%%%%%%%%%%INPUT REQUIREMENTS%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%The only input required for the algorithm is a matrix Psi
...
gap		=  abs((sum(log(w))+sum(log(Plam))))/(1+norm(sum(log(Plam)),inf));
mu              =  (lam'*y)/col;
...
% ***** TURN OFF INTERMEDIATE PRINTING **** %
...
%      - Psi*diag(ecol./y)*[mu*ecol - y.*lam]
%      + Psi*diag(lam./y)*[ecol - Psi'*w - y]
%
%   (= erow./w - mu*Psi*(ecol./y)   if ecol = y + Psi'*w),
%
% dy = ecol - y - Psi'w - Psi*dw  (= - Psi*dw if ecol=y+Psi'w), and
%
% dlam = mu*(ecol./y) - lam - (lam./y).*dy .
%
...
%Compute dy for the Newton step.
...
Ptw     =  Ptw - alfdual*dy;
normR   =  norm(R,inf);
gap     =  abs((sum(log(w))+sum(log(Plam))))...
                /(1+norm(sum(log(Plam)),inf));
